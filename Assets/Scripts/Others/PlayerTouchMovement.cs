...
{
    [SerializeField]
    private Vector2 JoystickSize = new(100, 100);
    [SerializeField]
    ...
    private void Update()
    {
        Vector3 scaledMovement = player.GetMoveSpeed() * Time.deltaTime * new Vector3(MovementAmount.x, 0, MovementAmount.y);
        player.HandleMovement(scaledMovement.normalized);
        player.HandleInteraction(scaledMovement.normalized);
        //player.transform.LookAt(player.transform.position + scaledMovement, Vector3.up);
        //player.transform.position = new Vector3(MovementAmount.x,0,MovementAmount.y);
